...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] Comenzar Caza', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	...
	Citizen.CreateThread(function()
		for index, v in ipairs(Positions) do
			if index ~= 'SpawnATV' then
				...
				AddTextComponentString('Punto de Caza')
...
function StartHuntingSession()
	...
	if OnGoingHuntSession then
		...
		Citizen.CreateThread(function()
			...
			local AnimalCoords = GenerateAnimalCoords( value.x, value.y, value.z)
				local Animal = CreatePed(5, GetHashKey('a_c_deer'),AnimalCoords, 0.0, true, false)
							...
							if PlyToAnimal < 2.0 then
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										ESX.ShowNotification('Usted necesita un cuchillo')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Has asesinado un ciervo y has recibido carne por: ' ..AnimalWeight.. 'kg')
...
function GenerateAnimalCoords(x,y,z)
	while true do
		...
		local coordZ = GetCoordZ(AnimalCoordX, AnimalCoordY)
